...
class View_first_round(tk.Toplevel):

    def __init__(self, tournament, serialized_round):
        tk.Toplevel.__init__(self)
        self.title("Premier tour du tournoi de {}".format(tournament))
        style = ttk.Style()
...
def view_round1():
    ...
    try:
        serialized_round[0]["round"] != "round1"
    except IndexError:
        showerror("Résultat", "Aucun premier tour en attente !")
    else:
        tournament = serialized_round[0]['tournament']
        View_first_round(tournament, serialized_round)
