...
class Players_class(tk.Toplevel):

    def __init__(self):
        tk.Toplevel.__init__(self)
        self.title("Liste des joueurs par classement")
        style = ttk.Style()
    ...
    def construct(self):
        ...
        tv.pack()
        #tv.pack(side='left')
        #vb = ttk.Scrollbar(self,
        #    orient=tk.VERTICAL, command=self.tv.yview)
        #vb.pack(side='right', fill='y')
        #tv.configure(yscrollcommand=vb.set)
        ...
        serialized_players = sorted(serialized_players, key=lambda k: k['classement'])
        i = 0
